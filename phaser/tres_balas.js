...
function update() {
    ...
    if ((Math.abs(jugador.position.x - 50) < 1) && (regresandoDer == false) && (estatusDerecha==1)){
    ...
    if(estatusAtras==1 && regresandoAtras == false){
       // despAtrTiempo++;
    }
    ...
    if(regresandoAtras==true && estatusAtras==1){
        jugador.position.x = Phaser.Math.linear(jugador.position.x, 50, 0.7);
    }

    if ((Math.abs(jugador.position.x - 50) < 1) && (regresandoAtras == true) && (estatusAtras==1)){
        jugador.position.x = 50;
        regresandoDer = false;
        estatusDerecha = 0;
        estatusIzquierda = 1;
...
function disparo(){
    velocidadBala =  -1 * velocidadRandom(200,400);
...
function disparo2(){
    velocidadBala2 =  1 * velocidadRandom(0,1);
    bala2.position.x = 60;
    bala2.position.y = 70;
    bala2.body.velocity.x = 0;
...
function disparo3(){
    ...
    bala3.visible = true;
    velocidadBala3y = 1 * velocidadRandom(0.05,0.5);
    velocidadBala3x = -640;
    bala3.position.x = 600;
    bala3.position.y = 100;
